...
export const onChatMessageCreate = functions
  ...
  .onCreate(async (snap) => {
    ...
    const otherUserDocumentReference = chatRoomData.users[0] == data.senderUserDocumentReference ? chatRoomData.users[1] : chatRoomData.users[0];
    ...
    const sendMessage = {
      ...
      uids: otherUserDocumentReference.id,
      id: otherUserDocumentReference.id,
    } as SendMessage;
